class Company < ActiveRecord::Base
	validates :name, :presence => {:message => "Company name cannot be blank"}, :format => { :with => /\A[a-z A-Z -]+\z/,
    		  :message => "Only letters allowed" }, :length => { :maximum => 40 }

    
	validates :website, :presence => {:message => "Website url cannot be blank"}, :length => { :maximum => 40 },
